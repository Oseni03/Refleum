...
/**
 * Retrieves custom feature prompts (cover letter, outreach) for an organization.
 * Prompts are stored in the organization's metadata field as JSON.
 */
export async function getOrgFeaturePrompts(orgId: string): Promise<Record<string, string>> {
    try {
        ...
        return {
            coverLetterPrompt: meta.coverLetterPrompt || "",
            outreachMessagePrompt: meta.outreachMessagePrompt || "",
        };
    } catch {
        return {};
    }
