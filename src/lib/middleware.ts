...
export async function requireApiKey(
	...
	if (!rawKey)
		return { valid: false, organizationId: null, error: "MISSING_KEY" };
...
/**
 * Enforce a required plan and return a ready-to-return `NextResponse` on failure,
 * or `null` when allowed. Standardizes the PLAN_REQUIRED response.
 */
export async function requirePlanOrError(
	organizationId: string,
	requiredPlan: keyof typeof PLAN_HIERARCHY = "STARTER",
): Promise<null | ReturnType<typeof NextResponse.json>> {
	const check = await requirePlan(organizationId, requiredPlan as any);
	if (!check.allowed) return apiError("PLAN_REQUIRED", 402);
	return null;
}
