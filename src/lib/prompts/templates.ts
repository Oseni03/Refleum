...
// Converted from Python templates.py — all LLM prompt strings for resume processing.
// Placeholders use {variable} syntax; callers do .replace("{variable}", value).
// NOTE: Unlike Python's .format(), JS .replace() is literal — {var} in JSON examples
//       is safe and does NOT need double-bracing.
...
export const IMPROVE_SCHEMA_EXAMPLE = `{
  "summary": "Experienced software engineer with 5+ years...",
  "workExperience": [
    {
      "id": 1,
...
const CRITICAL_TRUTHFULNESS_RULES_TEMPLATE = `CRITICAL TRUTHFULNESS RULES - NEVER VIOLATE:
...
function buildTruthfulnessRules(rule7: string): string {
    return CRITICAL_TRUTHFULNESS_RULES_TEMPLATE.replace("{rule_7}", rule7);
}

export const CRITICAL_TRUTHFULNESS_RULES: Record<string, string> = {
    nudge: buildTruthfulnessRules(
        "DO NOT add new bullet points or content - only rephrase existing content"
    ),
    keywords: buildTruthfulnessRules(
        "You may rephrase existing bullet points to include keywords, but do NOT add new bullet points"
    ),
    full: buildTruthfulnessRules(
...
const IMPROVE_RESUME_PROMPT_NUDGE = `Lightly nudge this resume toward the job description. Output ONLY the JSON object, no other text.
...
const IMPROVE_RESUME_PROMPT_KEYWORDS = `Enhance this resume with relevant keywords from the job description. Output ONLY the JSON object, no other text.
...
const IMPROVE_RESUME_PROMPT_FULL = `Tailor this resume for the job. Output ONLY the JSON object, no other text.
...
export const IMPROVE_RESUME_PROMPTS: Record<string, string> = {
...
export const GENERATE_TITLE_PROMPT = `Extract the job title and company name from this job description.
...
export const DIFF_STRATEGY_INSTRUCTIONS: Record<string, string> = {
    nudge: "Make minimal edits. Only rephrase where there is a clear match. Do not add new bullet points.",
    keywords:
        "Weave in relevant keywords where evidence already exists. You may rephrase bullets but do not add new ones.",
